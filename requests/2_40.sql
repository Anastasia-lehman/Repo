...
CREATE OR REPLACE VIEW student_hobby AS
with all_students as(select substr(st.n_group::varchar,1,1) as course, count(distinct(st.name)) as all_course
			        from students st
                    group by substr(st.n_group::varchar,1,1)), 
                    good_students as(select substr(st.n_group::varchar,1,1) as course, count(distinct(st.name)) as g_s
                                    from students st
                                    where st.score>4.5
                                    group by substr(st.n_group::varchar,1,1))
